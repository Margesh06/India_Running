...
"use client";
  ...
  const [paymentDetails, setPaymentDetails] = useState({
    ...
    const validateEmail = (email: string) => {
      ...
      const emailRegex = /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/i; // Fixed regex
    ...
    const validateEmergencyContact = (contact: string) => {
      ...
      if (!/^[A-Za-z\\s]+$/.test(contact)) return "Only letters and spaces are allowed";
      return "";
    };
  
    const validateEmergencyPhone = (phone: string) => {
      if (!phone) return "Emergency contact number is required";
      if (!/^[0-9]{10}$/.test(phone)) return "Phone number must be 10 digits";
      if (phone === formData.phone) return "Emergency contact number cannot be same as your number";
      return "";
    };
    ...
    const handleInputChange = (field: string, value: string | boolean) => {
      ...
      // Skip validation for boolean fields (e.g., checkboxes like terms and conditions)
    ...
    // Update form validation when formData or showAdditionalInfo changes
    useEffect(() => {
      setIsFormValid(validateForm());
    }, [formData, showAdditionalInfo]);
    ...
    const validateForm = () => {
      if (!showAdditionalInfo) {
        ...
        return !Object.values(personalInfoErrors).some(error => error !== "");
      } else {
        // Validate additional information
        const additionalInfoErrors = {
          emergencyContactName: validateEmergencyContact(formData.emergencyContactName),
          emergencyContactNumber: validateEmergencyPhone(formData.emergencyContactNumber),
          bloodGroup: formData.bloodGroup ? "" : "Blood group is required"
        };
  
        setErrors(prev => ({ ...prev, ...additionalInfoErrors }));
        return !Object.values(additionalInfoErrors).some(error => error !== "") && formData.termsAndCondition;
      }
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">Your registration for {eventName} has been confirmed.</p>
              ...
              <Link href={`/events/${eventId}`}>
                <Button className="w-full bg-[#00856F] hover:bg-[#00856F]/90 text-white">
                  Back to Home
                </Button>
              </Link>
            </div>
                    ...
                    <div className="space-y-4 p-2">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          className={`border-gray-300 \${errors.pincode ? 'border-red-500' : ''}`}
                        />
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          value={formData.dateOfBirth}
                          onChange={(e) => handleInputChange('dateOfBirth', e.target.value)}
                          className={`border-gray-300 \${errors.dateOfBirth ? 'border-red-500' : ''}`}
                      ...
                      <div>
                        ...
                        <RadioGroup
                          value={formData.gender}
                          onValueChange={(value) => handleInputChange('gender', value)}
                          className={`flex gap-6 mt-2 \${errors.gender ? 'border-red-500' : ''}`}
                        >
                          <div className="flex items-center gap-2">
                            <RadioGroupItem value="Male" id="male" className="border-gray-300" />
                            <Label htmlFor="male" className="text-sm">Male</Label>
                    ...
                    <div className="space-y-4 p-2">
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          placeholder="Enter Name" 
                          required 
                          className={`border p-3 w-full rounded bg-gray-100 text-gray-800 \${errors.emergencyContactName ? 'border-red-500' : ''}`}
                        />
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          placeholder="Enter Mobile No." 
                          required 
                          className={`border p-3 w-full rounded bg-gray-100 text-gray-800 \${errors.emergencyContactNumber ? 'border-red-500' : ''}`}
                        />
                      ...
                      <div>
                        ...
                        <select 
                          ...
                          value={formData?.bloodGroup || ""} 
                          onChange={(e) => handleInputChange('bloodGroup', e.target.value)} 
                          className={`border p-3 w-full rounded bg-gray-100 text-gray-800 \${errors.bloodGroup ? 'border-red-500' : ''}`}
                          required
                        >
                          <option value="" disabled>Select your blood group</option>
                          <option value="A+">A+</option>
